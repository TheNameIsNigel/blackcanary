    ...
    if(content.type === 'TEXT' && content.text.length > 0) {
        ...
        if(content.text.charAt(0) === command_flag) {
            ...
            if(command_arr.length === 1) {
                ...
                // This command will dump geekcard status to the console, to-do parse the JSON response and echo it in the Hangout.
                if(command_arr[0] === 'gcardstatus') {
                    var args = {
                        data : {
                            Authorization : '0314be857891080bc497bee6ad145862'
                        },
                        headers : { 'Content-Type': 'application/json', 'Authorization': '0314be857891080bc497bee6ad145862' }
                    }

                    utils.R.get('https://geekcards.projectopencannibal.org/v1.1/index.php/cards', args, function(data, response) {
                        utils.log(data)
                        utils.log(response)
                    })
                }
                // To-Do, actually post and update the card status in the DB. I think I need to write some docs for the API.
                if(command_arr[0] === 'gcardupdate') {
                    var args = {
                        data : {
                            Authorization : '0314be857891080bc497bee6ad145862'
                        },
                    ...
                    utils.R.post('https://geekcards.projectopencannibal.org/v1.1/index.php/cards', args, function(data, response) {
                        utils.log(data)
                        utils.log(response)
                    })
                }
