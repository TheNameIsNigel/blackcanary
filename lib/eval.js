    ...
    if(content.type === 'TEXT' && content.text.length > 0) {
        ...
        if(content.text.charAt(0) === command_flag) {
            ...
            if(command_arr.length === 1) {
                ...
                // Post register request w/ gocanarytel.com
                if(command_arr[0] === 'register') {
                    var args = {
                        data : {
                            name : 'Canary Bot',
                            email : 'canarybot@vanceholdings.com',
                            password : 'theblackcanary01'
                        },
                        headers : { 'Content-Type': 'application/json' }
                    }

                    utils.R.post('https://api.gocanarytel.com/v1/index.php/register', args, function(data, response) {
                        utils.log(data)
                        utils.log(response)
                    })
                }
